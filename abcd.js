function init() {
  ...
  const noWall = (actor) => {
    ...
    if (
      [...settings.bunnies.filter((el) => el.id !== actor.id), ...settings.elements].some((el) => {
    ...
    const buffer = 40;
    const noWallX = actor.move.x > 0 ? newPos.x + buffer < settings.map.w : newPos.x - buffer > 0;
    const noWallY = actor.move.y > 0 ? newPos.y < settings.map.h - buffer : newPos.y - buffer > 0;
  ...
  const handleWalk = () => {
    ...
    player.walkingInterval = setInterval(() => {
      ...
      player.move.x || player.move.y ? walk(player, dir) : stopSprite(player);
    }, 150);
  };
